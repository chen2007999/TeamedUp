...
public class Task extends Model{
    ...
    public static void addTask (Task task, Team team, User user) {
        if(find.where().eq("involvedEmail", user.getEmail()).findList().size() == 0) {
            Task addingTask = addTaskHelper(task, team, user);
            addingTask.setInvolvedEmail(user.getEmail());
            addingTask.save();
        }
    }

    public static Task addTaskHelper(Task task, Team team, User user) {
